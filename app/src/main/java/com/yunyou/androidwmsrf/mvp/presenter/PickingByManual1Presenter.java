...
@ActivityScope
public class PickingByManual1Presenter extends BasePresenter<PickingByManual1Contract.Model, PickingByManual1Contract.View> {
    @Inject
    ...
    @Inject
    public PickingByManual1Presenter(PickingByManual1Contract.Model model, PickingByManual1Contract.View rootView) {
    ...
    public void queryPickDetail(QueryPickDetailRequest request) {
        mModel.queryPickDetail(request)
                ...
                .subscribe(new ErrorHandleSubscriber<QueryPickDetailResponse>(mErrorHandler) {
                    @Override
                    public void onNext(QueryPickDetailResponse response) {
                        if (null == response) {
                        ...
                        if (response.isSuccess()) {
                            mRootView.goToPickingByManual2Activity(response.getBody());
