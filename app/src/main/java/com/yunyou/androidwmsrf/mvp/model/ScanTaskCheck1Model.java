...
@ActivityScope
public class ScanTaskCheck1Model extends BaseModel implements ScanTaskCheck1Contract.Model {
    @Inject
    Gson mGson;
    @Inject
    ...
    @Inject
    public ScanTaskCheck1Model(IRepositoryManager repositoryManager) {
    ...
    @Override
    public Observable<CheckTaskCountResponse> checkTaskCount(CheckTaskCountRequest request) {
        return Observable.just(mRepositoryManager
                .obtainRetrofitService(CommonService.class)
                .checkTaskCount(request))
                // 不使用缓存
