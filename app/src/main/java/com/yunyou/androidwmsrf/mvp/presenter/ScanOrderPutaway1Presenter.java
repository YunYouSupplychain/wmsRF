...
@ActivityScope
public class ScanOrderPutaway1Presenter extends BasePresenter<ScanOrderPutaway1Contract.Model, ScanOrderPutaway1Contract.View> {
    @Inject
    ...
    @Inject
    public ScanOrderPutaway1Presenter(ScanOrderPutaway1Contract.Model model, ScanOrderPutaway1Contract.View rootView) {
    ...
    public void queryPutAwayTaskByTaskNo(QueryPutAwayTaskByTaskNoRequest request) {
        mModel.queryPutAwayTaskByTaskNo(request)
                ...
                .subscribe(new ErrorHandleSubscriber<QueryPutAwayTaskByTaskNoResponse>(mErrorHandler) {
                    @Override
                    public void onNext(QueryPutAwayTaskByTaskNoResponse response) {
                        if (null == response) {
                        ...
                        if (response.isSuccess()) {
                            mRootView.goToScanOrderPutaway2Activity(response.getBody());
