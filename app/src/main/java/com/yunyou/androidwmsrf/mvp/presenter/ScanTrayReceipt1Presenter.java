...
@ActivityScope
public class ScanTrayReceipt1Presenter extends BasePresenter<ScanTrayReceipt1Contract.Model, ScanTrayReceipt1Contract.View> {
    @Inject
    ...
    @Inject
    public ScanTrayReceipt1Presenter(ScanTrayReceipt1Contract.Model model, ScanTrayReceipt1Contract.View rootView) {
    ...
    public void checkAsnIsPalletize(CheckAsnIsPalletizeRequest request) {
        mModel.checkAsnIsPalletize(request)
                ...
                .subscribe(new ErrorHandleSubscriber<CheckAsnIsPalletizeResponse>(mErrorHandler) {
                    @Override
                    public void onNext(CheckAsnIsPalletizeResponse response) {
                        if (null == response) {
                        ...
                        if (response.isSuccess()) {
                            mRootView.goToScanTrayReceipt2Activity();
