...
@ActivityScope
public class ScanRpTask1Presenter extends BasePresenter<ScanRpTask1Contract.Model, ScanRpTask1Contract.View> {
    @Inject
    ...
    @Inject
    public ScanRpTask1Presenter(ScanRpTask1Contract.Model model, ScanRpTask1Contract.View rootView) {
    ...
    public void queryReplenishmentDetail(QueryReplenishmentDetailRequest request) {
        mModel.queryReplenishmentDetail(request)
                ...
                .subscribe(new ErrorHandleSubscriber<QueryReplenishmentDetailResponse>(mErrorHandler) {
                    @Override
                    public void onNext(QueryReplenishmentDetailResponse response) {
                        if (null == response) {
                        ...
                        if (response.isSuccess()) {
                            mRootView.goToScanRpTask2Activity(response.getBody());
